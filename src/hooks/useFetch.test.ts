...
const mockData = { id: 1, name: 'Helado sandwich de nata', image: '/img/sandwich-nata.jpg', extraInfo: 'Caja 6 ud. (600ml)', price: 4.50 };
...
describe('useFetch', () => {
  test('devuelve loading=true inicialmente', () => {
    ...
    const { result } = renderHook(() => useFetch('https://api.test/products'));
  ...
  test('devuelve los datos correctamente tras el fetch', async () => {
    ...
    const { result } = renderHook(() => useFetch('https://api.test/products/1'));
  ...
  test('llama al endpoint pasado como argumento', async () => {
    ...
    const url = 'https://api.test/products/popular';
